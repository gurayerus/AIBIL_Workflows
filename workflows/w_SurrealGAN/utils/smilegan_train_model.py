...
def trainSmileGAN(train_data, nCluster, output_dir, covariate, start_fold, stop_fold, fold_number):
    ...
    Smile_GAN_clustering.cross_validated_clustering(train_data, nCluster, 
                                                    fold_number, 0.8, 
                                                    start_saving_epoch, max_epoch,
                                                    output_dir, WD, AQ, cluster_loss, 
                                                    'highest_matching_clustering', 
                                                    lam=9, mu = mu, batchSize=batchsize, 
                                                    verbose= False, beta1 = 0.5, 
                                                    lr = 0.0002, max_gnorm = 100, 
                                                    eval_freq = 10, save_epoch_freq = 10, 
...
if __name__ == '__main__':
    ...
    # Hyperparameter argument (optional)
    parser.add_argument('--ncluster', type=int, default=4,
                        help='Number of clusters for clustering (default: 4).')

    # Training fold arguments
